...
def generate_workflows(payload: dict[str, Any] | WorkflowGenerationRequest) -> WorkflowGenerationResponse:
    ...
    planner = _choose_planner(request)
    candidates = planner.generate_candidates(request, surface_map, service_map if isinstance(planner, LLMPlanner) else service_map.services)
    ...
    for candidate in candidates:
        ...
        if issues and isinstance(planner, LLMPlanner) and request.repair_attempts > 0:
            repaired = planner.repair_candidate(
                candidate,
...
def parse_request(payload: dict[str, Any]) -> WorkflowGenerationRequest:
    ...
    egress_domains = sorted(set(_string_list(payload.get("egress_domains")) + _infer_doc_domains(docs)))
...
def _choose_planner(request: WorkflowGenerationRequest):
    if request.planner == "rules":
        return RuleBasedPlanner()
    llm = LLMPlanner(model=request.planner_model)
    if request.planner == "llm":
        return llm
    return llm if llm.available() else RuleBasedPlanner()
