    ...
    for item in items:
        ...
        workflows.append(
            WorkflowDraft(
                ...
                test_fixtures=item.get("test_fixtures") if isinstance(item.get("test_fixtures"), dict) else {},
                ...
                quality_badges=_string_list(item.get("quality_badges")),
                run_readiness=item.get("run_readiness") if isinstance(item.get("run_readiness"), dict) else {},
...
def _build_generation_prompt(
    ...
    return (
        ...
        + focus_line +
        "Quality bar:\n"
        "- user_prompt must read as a realistic end-user request to the agent (\"Help me...\", \"Reconcile...\"), not meta commentary about testing.\n"
        "- Put user-visible intent, constraints, and desired outcome in user_prompt. Put exact interfaces, operations, fixtures, and cleanup mechanics in the hidden harness fields.\n"
        ...
        "user_prompt, task_prompt, required_secrets, egress_policy, seed_data, test_fixtures, expected_artifacts, expected_state_transitions,\n"
        "success_conditions, rubric, failure_modes_tested, difficulty, cleanup_required, quality_badges, run_readiness, selection_reason.\n\n"
        "Strict field requirements:\n"
        "- target_interfaces must use values from: cli, rest, sdk:python, sdk:javascript, mcp, browser.\n"
        "- user_prompt must be at least 20 words, sound like a user request, and must not mention hidden rubrics, fixtures, target interfaces, or implementation steps. It is the only task text sent to the evaluated agent.\n"
        "- task_prompt is an INTERNAL harness brief. It must name the exact documented interfaces and operations to evaluate, plus any setup and cleanup constraints; it is never sent to the evaluated agent.\n"
        ...
        "- If required_secrets is non-empty, declare an egress_policy route for the API domain being called.\n"
        "- test_fixtures must include literal URLs, filenames, content strings, storage keys, expected titles, or expected event types.\n"
        "- test_fixtures must include an operation, operations, create_operations, use_operations, cleanup_operations, or assertion_operations entry listing exact documented SDK calls, REST endpoints, CLI commands, MCP tools, or service routes used by task_prompt.\n"
        "- Do not output candidates whose only operation is a phrase like documented interface, product API, or available SDK.\n"
        "- expected_artifacts should name concrete proof objects like session_id, screenshot, file_content, trace_events, release_response.\n"
        "- If no service twins are declared, services, egress_policy, and seed_data should be empty lists.\n\n"
        "Example success_conditions:\n"
        "[\n"
        "  {\"description\": \"The trace shows the Python SDK created the documented resource before the browser action.\", \"evidence\": \"transcript\"},\n"
...
def _build_repair_prompt(
    ...
    return (
        ...
        "Preserve the visibility boundary: user_prompt is the realistic request sent to the evaluated agent; task_prompt, fixtures, rubrics, and success conditions are hidden harness data.\n"
...
def _parse_services(raw: Any, services_by_name: dict[str, ServiceDependency]) -> list[ServiceDependency]:
    ...
    for name in names:
        if name in services_by_name:
            services.append(services_by_name[name])
        else:
            services.append(ServiceDependency(name=name, mode="twin"))
    return services
